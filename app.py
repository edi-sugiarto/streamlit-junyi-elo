...
life_map = px.choropleth(df, locations='iso_alpha',
                         color='gdpPercap',
                         hover_name='country',
                         color_continuous_scale='viridis',
                         animation_frame='year', animation_group='country',
                         range_color=[100, 50000])
life_map.update_layout(transition_easing='linear')
st.write(life_map)
...
# covid2 = pd.read_csv('daily-new-confirmed-cases-of-covid-19-tests-per-case.csv')
# covid2.columns = ['Country', 'Code', 'Date', 'Confirmed', 'Days since confirmed']
# covid2['Date'] = pd.to_datetime(covid2['Date']).dt.strftime('%Y-%m-%d')
# covid_map = px.choropleth(covid2, locations='Code',
#                          color='Confirmed',
#                          hover_name='Country',
#                          color_continuous_scale='OrRd',
#                          animation_frame='Date', animation_group='Country',
#                          range_color=[100, 35000])
# covid_map.update_layout(transition_easing='linear')
# covid_map.layout.updatemenus[0].buttons[0].args[1]['frame']['duration'] = 100
# st.write(covid_map)